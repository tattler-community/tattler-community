...
from typing import Any, Mapping, Iterable, Tuple, Optional, Union
...
from tattler.client.tattler_py.tattler_client_utils import mk_correlation_id, get_endpoint_config, translate_attachments

log = logging.getLogger(__name__)
log.setLevel(os.getenv('LOG_LEVEL', 'info').upper())


def send_notification(scope: str, event: str, recipient: str, context: Optional[Mapping[str, Any]]=None, correlationId: Optional[str]=None, mode: str='debug', vectors: Optional[Iterable[str]]=None, priority: Optional[bool]=None, srv_addr: Optional[str]=None, srv_port: Optional[int]=None, attachments: Optional[Mapping[str, Union[Path, str]]]=None) -> Tuple[bool, Optional[Mapping[str, str]]]:
    ...
    :param attachments:     Optional dict of email attachments. Keys follow the
                            wire convention (``key@host`` for inline images,
                            otherwise the filename); values are either a
                            :class:`pathlib.Path` (local file, read by the
                            client) or an ``http(s)://`` URL string (fetched
                            by the server).
    ...
    if attachments is not None:
        if '_attachments' in context:
            raise ValueError(
                "Pass attachments via attachments= or context['_attachments'], not both")
        context['_attachments'] = translate_attachments(attachments)
